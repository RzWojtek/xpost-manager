...
🔧 AWARIA: brak nowych wpisów od 2026-08-13 — zmiana po stronie X (NAPRAWIONE)
Objaw

Od ~13.08.2026 18:11 aplikacja przestała dostawać nowe wpisy. Bot xparafbot działał (cron leci ~co 30 min, sprawdzał wszystkie konta), cookies ważne, ręczny „Pobierz z X" działał — ale przy każdym koncie log pokazywał „🌐 Otwieram… → ℹ️ Brak nowych postów", bez linii „✅ GraphQL".

Przyczyna (potwierdzona sondami, nie zgadywana)

X przemianował zapytanie GraphQL o timeline profilu z UserTweets na UserOriginalsTimeline. Bot nasłuchiwał tylko starych nazw (UserTweets/UserTweetsAndReplies/UserMedia/TweetDetail) → nowej odpowiedzi nie łapał → 0 tweetów. Ręczny import działał, bo używa OSOBNEGO parsera (TweetDetail).

Struktura nowej odpowiedzi: data.user.result.timeline (nie timeline_v2), __typename = User, w środku standardowe tweet_results. Parser bota parse_graphql_tweets już to obsługuje (znalazł 5 tweetów w teście).

Jak zdiagnozowano (metoda na przyszłość, gdyby X znów coś zmienił)

Sondy Playwright w venv bota (cd /root/xparafbot && source venv/bin/activate):

diag_x.py <konto> — jakie odpowiedzi GraphQL wpadają + czy 200/błąd.
diag_x2.py <konto> — lista WSZYSTKICH operacji GraphQL + test sesji przez wejście na x.com/home (czy zalogowany). Wynik: sesja zalogowana, wpadł UserByScreenName (200) ale NIE UserTweets — za to UserOriginalsTimeline.
diag_x3.py <konto> — łapie UserOriginalsTimeline i puszcza na nim parse_graphql_tweets bota → potwierdziło, że parser działa (fix = tylko nazwa). (Cookies do sond ładować jak bot: tylko name/value/domain/path.)
Fix (1 linia)

W /root/xparafbot/xparafbot.py, funkcja fetch_posts_playwright, lista TWEET_ENDPOINTS — dodano 'UserOriginalsTimeline' na początek. Parser bez zmian. Wgranie: WinSCP → /root/xparafbot/, potem żeby nadgonić od razu: cd /root/xparafbot && source venv/bin/activate && python3 xparafbot.py. W logu wrócił „✅ GraphQL (UserOriginalsTimeline): N nowych postów".

Ważne fakty o xparafbot (do zapamiętania)
NIE jest procesem PM2 — leci z crona (~co 30 min), katalog /root/xparafbot/, własny venv.
Cookies: /root/xparafbot/x_cookies.json, używane jako name/value/domain/path (bez sameSite).
Pisze do Arkusza Google (append-only), skąd syncSheets mostkuje do Firebase.
💡 CO JESZCZE MOŻNA POBRAĆ Z X (pod przyszłe funkcje)

Kluczowe: niemal wszystko poniżej jest już w tej samej odpowiedzi UserOriginalsTimeline (tweet_results.result → legacy / core / views / note_tweet). Dodanie = tylko więcej parsowania w parse_graphql_tweets, bez nowych zapytań do X (bez ryzyka limitów). Ścieżka wdrożenia dla każdej: parse_graphql_tweets (bot) → kolumna w Arkuszu → pole w Firebase → wyświetlenie na karcie.

Metryki (do sortowania kolejki „co zaskoczyło"): favorite_count (❤), retweet_count (🔁), reply_count (💬), quote_count, bookmark_count (🔖), views.count (👁).

Encje (pod krypto): entities.symbols = cashtagi $TICKER (→ DexScreener), entities.hashtags (→ auto-tagi), entities.user_mentions (→ mapa @kont).

Autor (core.user_results.result.legacy): followers_count (→ alpha-score kont), created_at konta = wiek (→ wykrywanie scamu: świeże konto + prośba/ref), verified/is_blue_verified, description (bio), profile_image_url_https (avatar).

Media: extended_entities.media[].type = photo/video/animated_gif (teraz łapiemy tylko photo) + video_info.variants[].url.

Kontekst: lang (język), is_quote_status+quoted_status_result (cytowany wpis), in_reply_to_screen_name (odpowiedź do kogo), source (klient — wykrywanie botów).

Wymaga OSOBNYCH zapytań (droższe, uwaga na limity): wyszukiwarka po tickerze (SearchTimeline), lista followersów, trendy, kto polubił/podał dalej.

Gdzie na karcie (jest mockup karta_mockup.html): avatar+autor→nagłówek, metryki→pasek pod nagłówkiem, flaga scamu→góra, cashtagi/hashtagi→pod treścią. Priorytet: metryki + cashtagi + dane autora (największy zysk, zero kosztu zapytań).

🤖 ODŚWIEŻENIE MODELI AI (v2.48) — martwe modele wymienione

Objaw: „Błąd AI: Wszystkie modele wyczerpały limity" przez 2 dni. To nie był realny limit — dostawcy wycofali modele (te darmowe listy zmieniają się prawie co miesiąc), a wywołanie nieistniejącego modelu apka pokazywała jako „limit".

Jak sprawdzić klucz/model BEZ logowania do dostawcy: klucz z Vercel (Settings → Environment Variables → reveal), potem curl <api>/models -H "Authorization: Bearer KEY" (lista modeli = klucz OK; 401 = zły klucz). Dla Groq: https://api.groq.com/openai/v1/models.

Nowa tablica AI_MODELS (7):

Groq llama-3.3-70b-versatile — żyje (bez zmian)
Gemini gemini-2.5-flash — 2.0 wycofany (zmiana też w wywołaniu Vision/OCR ~linia 6398)
Cerebras gpt-oss-120b — zamiast martwego llama3.1-8b
SambaNova Meta-Llama-3.3-70B-Instruct — zamiast 3.1 (do potwierdzenia curl-em)
OpenRouter meta-llama/llama-4-scout:free — zamiast martwego deepseek-r1:free (DeepSeek na OpenRouter przeszedł na płatne)
OpenRouter meta-llama/llama-4-maverick:free — odświeżony (był llama-3.3-70b-instruct:free)
Groq openai/gpt-oss-120b — zamiast martwego mixtral-8x7b-32768

Gotcha: apka liczy zużycie Groq w localStorage['groqUsage_v1'] — mogła się „zablokować" przez martwy model. Reset: F12 → localStorage.removeItem('groqUsage_v1') → odśwież. Warto nie polegać na jednym modelu (apka i tak leci fallbackiem po liście).

Data wpisu: 2026-08-15. X potrafi znów zmienić nazwy operacji — metoda diagnozy: sondy diag_x*.py.
...
# ════════════════════════════════════════════════════════════════
# CONTEXT.md — wpis: awaria pobierania z X (fix) + dane z X + modele AI
# (dopisz jako kolejny punkt na końcu CONTEXT.md)
# ════════════════════════════════════════════════════════════════

## 🔧 AWARIA: brak nowych wpisów od 2026-08-13 — zmiana po stronie X (NAPRAWIONE)

### Objaw
Od ~13.08.2026 18:11 aplikacja przestała dostawać nowe wpisy. Bot `xparafbot`
**działał** (cron leci ~co 30 min, sprawdzał wszystkie konta), cookies **ważne**,
ręczny „Pobierz z X" **działał** — ale przy każdym koncie log pokazywał
„🌐 Otwieram… → ℹ️ Brak nowych postów", bez linii „✅ GraphQL".

### Przyczyna (potwierdzona sondami, nie zgadywana)
**X przemianował zapytanie GraphQL o timeline profilu z `UserTweets` na
`UserOriginalsTimeline`.** Bot nasłuchiwał tylko starych nazw
(`UserTweets`/`UserTweetsAndReplies`/`UserMedia`/`TweetDetail`) → nowej odpowiedzi
nie łapał → 0 tweetów. Ręczny import działał, bo używa OSOBNEGO parsera (`TweetDetail`).

Struktura nowej odpowiedzi: `data.user.result.timeline` (nie `timeline_v2`),
`__typename = User`, w środku standardowe `tweet_results`. **Parser bota
`parse_graphql_tweets` już to obsługuje** (znalazł 5 tweetów w teście).

### Jak zdiagnozowano (metoda na przyszłość, gdyby X znów coś zmienił)
Sondy Playwright w venv bota (`cd /root/xparafbot && source venv/bin/activate`):
1. `diag_x.py <konto>` — jakie odpowiedzi GraphQL wpadają + czy 200/błąd.
2. `diag_x2.py <konto>` — lista WSZYSTKICH operacji GraphQL + test sesji przez
   wejście na `x.com/home` (czy zalogowany). Wynik: sesja zalogowana, wpadł
   `UserByScreenName` (200) ale NIE `UserTweets` — za to `UserOriginalsTimeline`.
3. `diag_x3.py <konto>` — łapie `UserOriginalsTimeline` i puszcza na nim
   `parse_graphql_tweets` bota → potwierdziło, że parser działa (fix = tylko nazwa).
   (Cookies do sond ładować jak bot: tylko `name/value/domain/path`.)

### Fix (1 linia)
W `/root/xparafbot/xparafbot.py`, funkcja `fetch_posts_playwright`, lista
`TWEET_ENDPOINTS` — dodano `'UserOriginalsTimeline'` na początek. Parser bez zmian.
Wgranie: WinSCP → `/root/xparafbot/`, potem żeby nadgonić od razu:
`cd /root/xparafbot && source venv/bin/activate && python3 xparafbot.py`.
W logu wrócił „✅ GraphQL (UserOriginalsTimeline): N nowych postów".
...
Kluczowe: **niemal wszystko poniżej jest już w tej samej odpowiedzi
`UserOriginalsTimeline`** (`tweet_results.result` → `legacy` / `core` / `views` /
`note_tweet`). Dodanie = tylko więcej parsowania w `parse_graphql_tweets`,
**bez nowych zapytań do X** (bez ryzyka limitów). Ścieżka wdrożenia dla każdej:
`parse_graphql_tweets` (bot) → kolumna w Arkuszu → pole w Firebase → wyświetlenie na karcie.

**Metryki (do sortowania kolejki „co zaskoczyło"):** `favorite_count` (❤),
`retweet_count` (🔁), `reply_count` (💬), `quote_count`, `bookmark_count` (🔖),
`views.count` (👁).

**Encje (pod krypto):** `entities.symbols` = **cashtagi $TICKER** (→ DexScreener),
`entities.hashtags` (→ auto-tagi), `entities.user_mentions` (→ mapa @kont).

**Autor (`core.user_results.result.legacy`):** `followers_count` (→ alpha-score kont),
`created_at` konta = wiek (→ **wykrywanie scamu**: świeże konto + prośba/ref),
`verified`/`is_blue_verified`, `description` (bio), `profile_image_url_https` (**avatar**).

**Media:** `extended_entities.media[].type` = photo/**video**/**animated_gif**
(teraz łapiemy tylko photo) + `video_info.variants[].url`.

**Kontekst:** `lang` (język), `is_quote_status`+`quoted_status_result` (cytowany wpis),
`in_reply_to_screen_name` (odpowiedź do kogo), `source` (klient — wykrywanie botów).

**Wymaga OSOBNYCH zapytań (droższe, uwaga na limity):** wyszukiwarka po tickerze
(`SearchTimeline`), lista followersów, trendy, kto polubił/podał dalej.

**Gdzie na karcie** (jest mockup `karta_mockup.html`): avatar+autor→nagłówek,
metryki→pasek pod nagłówkiem, flaga scamu→góra, cashtagi/hashtagi→pod treścią.
Priorytet: metryki + cashtagi + dane autora (największy zysk, zero kosztu zapytań).

---

## 🤖 ODŚWIEŻENIE MODELI AI (v2.48) — martwe modele wymienione

**Objaw:** „Błąd AI: Wszystkie modele wyczerpały limity" przez 2 dni. To nie był
realny limit — dostawcy **wycofali modele** (te darmowe listy zmieniają się prawie
co miesiąc), a wywołanie nieistniejącego modelu apka pokazywała jako „limit".

**Jak sprawdzić klucz/model BEZ logowania do dostawcy:** klucz z Vercel (Settings →
Environment Variables → reveal), potem `curl <api>/models -H "Authorization: Bearer KEY"`
(lista modeli = klucz OK; 401 = zły klucz). Dla Groq: `https://api.groq.com/openai/v1/models`.

**Nowa tablica `AI_MODELS` (7):**
1. Groq `llama-3.3-70b-versatile` — żyje (bez zmian)
2. Gemini `gemini-2.5-flash` — 2.0 wycofany (zmiana też w wywołaniu Vision/OCR ~linia 6398)
3. Cerebras `gpt-oss-120b` — zamiast martwego `llama3.1-8b`
4. SambaNova `Meta-Llama-3.3-70B-Instruct` — zamiast `3.1` (do potwierdzenia curl-em)
5. OpenRouter `meta-llama/llama-4-scout:free` — zamiast martwego `deepseek-r1:free` (DeepSeek na OpenRouter przeszedł na płatne)
6. OpenRouter `meta-llama/llama-4-maverick:free` — odświeżony (był `llama-3.3-70b-instruct:free`)
7. Groq `openai/gpt-oss-120b` — zamiast martwego `mixtral-8x7b-32768`

**Gotcha:** apka liczy zużycie Groq w `localStorage['groqUsage_v1']` — mogła się
„zablokować" przez martwy model. Reset: F12 → `localStorage.removeItem('groqUsage_v1')` → odśwież.
Warto nie polegać na jednym modelu (apka i tak leci fallbackiem po liście).
